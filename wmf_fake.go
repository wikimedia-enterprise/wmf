...
type APIFake struct {
	...
	entities   map[string]*WikibaseEntity
}

func (f *APIFake) GetWikidataEntityCreationDate(ctx context.Context, entityTitle string) (*time.Time, error) {
	now := time.Now()
	return &now, nil
}
...
func NewAPIFake() *APIFake {
	return &APIFake{
		...
		entities:  make(map[string]*WikibaseEntity),
	}
}
...
func (f *APIFake) InsertEntity(id string, entity *WikibaseEntity) {
...
func (f *APIFake) GetWikibaseEntity(ctx context.Context, dtb, entityID string, ops ...func(*url.Values)) (*WikibaseEntityResponse, error) {

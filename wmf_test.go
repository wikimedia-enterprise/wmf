...
const getWikidataCreationDateResponse = `{
  "batchcomplete": true,
  "query": {
    "pages": {
      "123": {
...
const getWikidataCreationDateMissingResponse = `{
  "batchcomplete": true,
  "query": {
    "pages": {
      "-1": {
...
//go:embed testdata/get_wikibase_entity_q2.json
var getWikibaseEntityResponseQ2 string

//go:embed testdata/get_wikibase_entity_missing.json
var getWikibaseEntityResponseMissing string

//go:embed testdata/get_wikibase_entity_error.json
var getWikibaseEntityResponseError string
...
func createWikiDataTestServer(status int, payloads ...string) *httptest.Server {
	rtr := http.NewServeMux()

	nextResponse := 0

	rtr.HandleFunc("/wiki/Special:EntityData/", func(w http.ResponseWriter, r *http.Request) {
		path := r.URL.Path
...
func (s *getLanguagesTestSuite) SetupSuite() {
	...
	if rsp.SiteMatrix != nil {
		...
		for _, prj := range rsp.SiteMatrix.Specials {
			if prj.DBName == "commonswiki" || prj.DBName == "wikidatawiki" {
...
func TestGetProjects(t *testing.T) {
	for _, testCase := range []*getProjectsTestSuite{
		...
		},
		{
			...
			hasError: true,
			dtb:      "not found",
			err:      errors.New("error creating request for languages, couldn't fetch project data: project not found"),
		},
		{
			sts:      http.StatusOK,
			pld:      errorResponse,
...
type getWikidataEntityCreationDateTestSuite struct {
	suite.Suite
	ctx        context.Context
	srv        *httptest.Server
	clt        *Client
...
func (s *getWikidataEntityCreationDateTestSuite) SetupSuite() {
	s.srv = createActionsAPIServer(s.sts, s.pld)
	s.ctx = context.Background()
	s.clt = &Client{
		DefaultURL:      s.srv.URL,
		DefaultDatabase: "wikidatawiki",
		HTTPClient:      &http.Client{},
		Tracer:          mockTracer,
	}
}

func (s *getWikidataEntityCreationDateTestSuite) TearDownSuite() {
	s.srv.Close()
}

func (s *getWikidataEntityCreationDateTestSuite) TestGetWikidataEntityCreationDate() {
	ts, err := s.clt.GetWikidataEntityCreationDate(s.ctx, s.entity)

	if s.expectErr != nil {
		s.Assert().Error(err)
		s.Assert().Contains(err.Error(), s.expectErr.Error())
		s.Assert().Nil(ts)
		return
	}
	...
	s.Assert().NoError(err)
	s.Assert().NotNil(ts)
	s.Assert().True(ts.Equal(*s.expectTime))
}
...
type getWikibaseEntityTestSuite struct {
	suite.Suite
	ctx          context.Context
	srv          *httptest.Server
	clt          *Client
...
func (s *getWikibaseEntityTestSuite) SetupSuite() {
	s.srv = createWikiDataTestServer(s.sts, s.pld)
	s.dtb = "wikidatawiki"
	s.ctx = context.Background()
	s.clt = &Client{
		DefaultURL:      s.srv.URL,
...
func (s *getWikibaseEntityTestSuite) TearDownSuite() {
	s.srv.Close()
}

func (s *getWikibaseEntityTestSuite) TestGetWikibaseEntity() {
	rsp, err := s.clt.GetWikibaseEntity(s.ctx, s.dtb, s.entityID)
	...
	if s.hasError {
		...
		s.Assert().NoError(err)
		s.Assert().NotNil(rsp)
		s.Assert().NotNil(rsp.Entities)

		if s.expectedID != "" {
			entity, ok := rsp.Entities[s.expectedID]
			s.Assert().True(ok, "Entity should exist in response")
			s.Assert().NotNil(entity)

			if s.expectedType != "" {
				s.Assert().Equal(s.expectedType, entity.Type)
			}

			s.Assert().Equal(s.expectedID, entity.ID)
		}
	}
}

func TestGetWikibaseEntity(t *testing.T) {
	for _, testCase := range []*getWikibaseEntityTestSuite{
		{
			sts:          http.StatusOK,
			pld:          getWikibaseEntityResponseQ2,
			entityID:     "Q2",
			expectedID:   "Q2",
			expectedType: "item",
			hasError:     false,
		...
		{
			sts:      http.StatusOK,
			pld:      getWikibaseEntityResponseMissing,
			entityID: "Q999999999",
			hasError: true,
		...
		{
			sts:      http.StatusOK,
			pld:      getWikibaseEntityResponseError,
			entityID: "INVALID",
			hasError: true,
		},
		{
...
type getWikibaseEntityDetailsTestSuite struct {
	suite.Suite
	ctx      context.Context
	srv      *httptest.Server
	clt      *Client
...
func (s *getWikibaseEntityDetailsTestSuite) SetupSuite() {
	s.srv = createWikiDataTestServer(http.StatusOK, getWikibaseEntityResponseQ2)
	s.dtb = "wikidatawiki"
	s.ctx = context.Background()
	s.clt = &Client{
		DefaultURL:      s.srv.URL,
...
func (s *getWikibaseEntityDetailsTestSuite) TearDownSuite() {
	s.srv.Close()
}

func (s *getWikibaseEntityDetailsTestSuite) TestEntityStructure() {
	rsp, err := s.clt.GetWikibaseEntity(s.ctx, s.dtb, s.entityID)

	s.Assert().NoError(err)
	s.Assert().NotNil(rsp)
	s.Assert().Equal(1, rsp.Success)

	entity := rsp.Entities[s.entityID]
	s.Assert().NotNil(entity)

	// Test labels
	s.Assert().NotNil(entity.Labels)
	s.Assert().Contains(entity.Labels, "en")
	s.Assert().Equal("Earth", entity.Labels["en"].Value)
	s.Assert().Equal("en", entity.Labels["en"].Language)

	// Test descriptions
	s.Assert().NotNil(entity.Descriptions)
	s.Assert().Contains(entity.Descriptions, "en")
	s.Assert().Equal("third planet from the Sun in the Solar System", entity.Descriptions["en"].Value)

	// Test aliases
	s.Assert().NotNil(entity.Aliases)
	s.Assert().Contains(entity.Aliases, "en")
	s.Assert().GreaterOrEqual(len(entity.Aliases["en"]), 1)
	s.Assert().Equal("Planet Earth", entity.Aliases["en"][0].Value)

	// Test claims
	s.Assert().NotNil(entity.Claims)
	s.Assert().Contains(entity.Claims, "P31")
	s.Assert().GreaterOrEqual(len(entity.Claims["P31"]), 1)

	claim := entity.Claims["P31"][0]
	s.Assert().NotEmpty(claim.ID)
	s.Assert().Equal("statement", claim.Type)
	s.Assert().Equal("normal", claim.Rank)
	s.Assert().NotNil(claim.MainSnak)
	s.Assert().Equal("P31", claim.MainSnak.Property)
	s.Assert().Equal("value", claim.MainSnak.SnakType)

	// Test sitelinks
	s.Assert().NotNil(entity.Sitelinks)
	s.Assert().Contains(entity.Sitelinks, "enwiki")
	s.Assert().Equal("Earth", entity.Sitelinks["enwiki"].Title)
	s.Assert().Equal("https://en.wikipedia.org/wiki/Earth", entity.Sitelinks["enwiki"].URL)

	// Test metadata
	s.Assert().Equal(2345678901, entity.LastRevID)
	s.Assert().Equal("2024-10-15T12:34:56Z", entity.Modified)
	s.Assert().Equal(123, entity.PageID)
}

func TestGetWikibaseEntityDetails(t *testing.T) {
	suite.Run(t, new(getWikibaseEntityDetailsTestSuite))
}

type getWikibaseEntityWithOptionsTestSuite struct {
	suite.Suite
	ctx      context.Context
	srv      *httptest.Server
	clt      *Client
...
func (s *getWikibaseEntityWithOptionsTestSuite) SetupSuite() {
	s.srv = createWikiDataTestServer(http.StatusOK, getWikibaseEntityResponseQ2)
	s.dtb = "wikidatawiki"
	s.ctx = context.Background()
	s.clt = &Client{
...
func (s *getWikibaseEntityWithOptionsTestSuite) TearDownSuite() {
	s.srv.Close()
}

func (s *getWikibaseEntityWithOptionsTestSuite) TestGetWikibaseEntityWithOptions() {
	// Test with custom options
	opts := func(v *url.Values) {
		v.Set("props", "labels|descriptions")
		v.Set("languages", "en")
	}

	rsp, err := s.clt.GetWikibaseEntity(s.ctx, s.dtb, s.entityID, opts)

	s.Assert().NoError(err)
	s.Assert().NotNil(rsp)
	s.Assert().Equal(1, rsp.Success)
	s.Assert().Contains(rsp.Entities, s.entityID)
}

func TestGetWikibaseEntityWithOptions(t *testing.T) {
	suite.Run(t, new(getWikibaseEntityWithOptionsTestSuite))
}

func TestGetWikidataEntityCreationDate(t *testing.T) {
	parsedTime, _ := time.Parse(time.RFC3339, "2012-03-04T05:06:07Z")

	for _, testCase := range []*getWikidataEntityCreationDateTestSuite{
		{
			entity:     "Q123",
			pld:        getWikidataCreationDateResponse,
			sts:        http.StatusOK,
			expectTime: &parsedTime,
		},
		{
			entity:    "Q999",
			pld:       getWikidataCreationDateMissingResponse,
			sts:       http.StatusOK,
			expectErr: ErrEntityNotFound,
		},
		{
			entity:    "Q123",
			sts:       http.StatusInternalServerError,
